...
let MgtPeoplePicker = class MgtPeoplePicker extends MgtTemplatedComponent {
    ...
    renderSearchResults(people) {
      ...
      >
         ${repeat(filteredPeople, person => person.id, person => {
            const lineTwo = person.jobTitle || person.mail;
            const ariaLabel = `${this.strings.suggestedContact} ${person.displayName} ${lineTwo !== null && lineTwo !== void 0 ? lineTwo : ''}`;
            return html `
               <li
                id="${person.id}"
    ...
    handleArrowSelection(event) {
        ...
        if (peopleList && peopleList.children.length) {
            ...
            for (const person of peopleList === null || peopleList === void 0 ? void 0 : peopleList.children) {
                const p = person;
